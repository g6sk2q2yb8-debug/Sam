...
# استخدام متغير البيئة للتوكن أو التوكن المباشر
TOKEN = os.environ.get("TOKEN", "8647831819:AAFo8_JxQXN5uMAdIlN458ja1bsL_G15q94")
...
def handle_youtube(message):
    ...
    try:
        ...
        # تحميل أعلى جودة متاحة بصيغة mp4 مباشرة بدون الحاجة لـ ffmpeg
        ydl_opts = {
        ...
        if downloaded_file and os.path.exists(downloaded_file):
            ...
            if file_size_mb <= 50:
                ...
                bot.edit_message_text("❌ حجم الفيديو يتجاوز الحد المسموح به (2GB).", message.chat.id, wait_msg.message_id)
        else:
            bot.edit_message_text("❌ لم يتم العثور على الملف المحمل.", message.chat.id, wait_msg.message_id)

    except yt_dlp.DownloadError as e:
        bot.edit_message_text(f"❌ حدث خطأ أثناء التحميل: {str(e)}", message.chat.id, wait_msg.message_id)
